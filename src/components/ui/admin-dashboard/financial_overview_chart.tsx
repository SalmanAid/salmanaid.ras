...
export default function AdminDashboard_FinancialOverviewChart() {
  ...
  const totals = chartData.reduce(
    (sum, row) => ({
      ...
      disbursement: sum.disbursement + row.disbursement,
    }),
    { donations: 0, disbursement: 0 }
  )
    ...
    <div className="w-full">
      ...
      <div className="flex flex-wrap justify-center items-center gap-4 mt-3">
        <div className="flex items-center gap-2 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2">
          ...
          <span className="text-xs font-semibold text-gray-800">{formatTooltipValue(totals.donations)}</span>
        </div>
        <div className="flex items-center gap-2 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2">
          ...
          <span className="text-xs font-semibold text-gray-800">{formatTooltipValue(totals.disbursement)}</span>
        </div>
      </div>
    </div>
  )
